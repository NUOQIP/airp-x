...
function CommentItem({ comment, all, snapshot, onReply, depth = 0 }: { comment: Comment; all: Comment[]; snapshot: NonNullable<ReturnType<typeof useSnapshot>["data"]>; onReply: (comment: Comment) => void; depth?: number }) {
  ...
  return <div className={depth ? "ml-10 border-l-2 border-line pl-2" : ""}>
    ...
    {depth < 5 && children.map((child) => <CommentItem key={child.id} comment={child} all={all} snapshot={snapshot} onReply={onReply} depth={depth + 1} />)}
  </div>;
}
...
export function PostPage() {
  ...
  const submit = () => {
    ...
    turn.mutate({ kind: "comment", branchId: data.session.activeBranchId, postId: post.id, ...(parent ? { parentCommentId: parent.id } : {}), text: value });
  };
  return <div>
    <header className="sticky top-0 z-20 flex h-[53px] items-center gap-7 border-b border-line bg-white/90 px-3 backdrop-blur-md"><button className="x-icon-button" onClick={() => navigate(-1)}><ArrowLeft size={20} /></button><h1 className="panel-title">帖文</h1></header>
    ...
    <section className="border-b border-line px-4 py-3">
      ...
      <div className="flex gap-3"><Avatar name={player.displayName} seed={player.avatarSeed} text={player.avatarText} url={player.avatarUrl} /><div className="flex-1"><textarea className="min-h-20 w-full resize-none border-0 py-2 text-lg outline-none placeholder:text-muted" placeholder="发布你的回复" value={text} onChange={(event) => setText(event.target.value)} /><div className="flex items-center justify-between border-t border-line pt-2"><span className={`text-xs ${text.length > 12_000 ? "text-rose-500" : "text-muted"}`}>{text.length}/12000</span><button className="rounded-full bg-accent px-5 py-2 font-bold text-white disabled:opacity-50" disabled={!text.trim() || text.length > 12_000 || turn.isPending} onClick={submit}>{turn.isPending ? "生成中…" : "回复"}</button></div></div></div>
      {turn.error && <div className="mt-3 rounded-xl bg-rose-50 p-3 text-sm text-rose-700">{turn.error.message}<div className="mt-1 text-xs">你的回复已经保存，可以在右侧会话状态中重试。</div></div>}
    </section>
    {comments.filter((item) => !item.parentId).map((comment) => <CommentItem key={comment.id} comment={comment} all={comments} snapshot={data} onReply={setReplyTo} />)}
