...
import { Info, LockKeyhole, Mail, Pencil, Search, Send } from "lucide-react";
...
export function MessagesPage() {
  ...
  const messages = data?.messages.filter((message) => message.threadId === selected?.id).sort((a, b) => a.createdAt.localeCompare(b.createdAt)) ?? [];
  ...
  const submit = () => {
    if (!text.trim() || !selected.playerCanSend) return;
    const value = text.trim();
    const replyId = replyToId;
    setText("");
    setReplyToId(undefined);
    turn.mutate({ kind: selected.kind, branchId: data.session.activeBranchId, threadId: selected.id, ...(replyId ? { replyToMessageId: replyId } : {}), text: value });
  };
  return <div className="grid h-screen grid-cols-[255px_1fr] overflow-hidden">
    <section className="border-r border-line">
      ...
      <div className="mx-3 mb-2 flex items-center gap-2 rounded-full bg-slate-100 px-3 py-2 text-muted"><Search size={16} /><input aria-label="搜索私信" className="w-full bg-transparent text-sm outline-none" placeholder="搜索私信" /></div>
      <div className="h-[calc(100vh-103px)] overflow-y-auto">
        {threads.map((thread) => {
          ...
          const last = data.messages.filter((message) => message.threadId === thread.id).at(-1);
    ...
    <section className="flex min-w-0 flex-col">
      ...
      {selected.playerCanSend ? <div className="shrink-0 border-t border-line p-3">
        ...
        {turn.error && <div className="mb-2 rounded-lg bg-rose-50 px-3 py-2 text-xs text-rose-700">{turn.error.message}（消息已保存，可从右侧重试）</div>}
        <div className="flex items-end gap-2 rounded-2xl border border-slate-300 px-3 py-2 focus-within:border-accent"><textarea value={text} onChange={(event) => setText(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); submit(); } }} className="max-h-32 min-h-8 flex-1 resize-none border-0 bg-transparent outline-none" placeholder="开始一条新消息" /><button className="x-icon-button text-accent" disabled={!text.trim() || turn.isPending} onClick={submit}><Send size={19} /></button></div>
