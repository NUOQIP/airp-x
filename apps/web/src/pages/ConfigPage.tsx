...
function RulePreset({ config }: { config: ConfigSnapshot }) {
  ...
  return <div><SectionTitle title="玩法规则预设（YAML）" detail="这是始终生效的全局规则。保存时会解析 YAML；下方硬约束由 hard_constraints 自动同步并在 AI Schema 通过后继续验证。" /><label className="block text-sm font-bold">YAML 规则<textarea className="x-input mt-1 min-h-[520px] font-mono text-xs leading-5" value={draft.rawText} onChange={(event) => setDraft({ ...draft, rawText: event.target.value })} /></label><div className="mt-4 grid grid-cols-4 gap-3"><div className="rounded-xl bg-slate-50 p-3"><div className="text-xs font-bold text-muted">固定主页变化</div><div className="mt-1 text-xl font-extrabold">关闭</div></div><div className="rounded-xl bg-slate-50 p-3"><div className="text-xs font-bold text-muted">主页刷新演出</div><div className="mt-1 text-base font-extrabold">仅真实变化</div></div><div className="rounded-xl bg-slate-50 p-3"><div className="text-xs font-bold text-muted">最多面板</div><div className="mt-1 text-xl font-extrabold">{draft.maxPanels}</div></div><div className="rounded-xl bg-slate-50 p-3"><div className="text-xs font-bold text-muted">每条新帖评论目标</div><div className="mt-1 text-xl font-extrabold">{draft.representativeComments}</div></div></div><p className="mt-2 text-xs leading-5 text-muted">主页不再为凑数量强制变化或生成空面板；评论数量是生成目标，至少有一条有效评论即可提交；其他参数请在 YAML 的 hard_constraints 中修改。</p><button className="x-primary mt-4 flex items-center gap-2" onClick={() => save.mutate()}><Save size={17} />校验并保存 YAML</button>{save.error && <p className="mt-2 text-sm text-rose-600">{save.error.message}</p>}</div>;
}
