...
const stateWriteContractV2: PromptMessage = {
  ...
  content: [
    ...
    "For ordinary commenters, reuse the supplied audience-pool candidates for most comments. Reference an existing candidate directly by comment.authorId and never account.upsert it again. Introduce no more than the configured maximum of new commenter accounts; successful newcomers enter the pool. Identity categories are long-term possibilities, not a checklist to cover in one thread.",
    "Do not encode a commenter's current narrative function directly in displayName, handle, bio, or id. When a thread reaches the configured threshold, use parentId for the required share of comments so users reply to one another instead of all addressing the author independently.",
  ...
  const messages: PromptMessage[] = [
    {
      ...
      content: [
        ...
        "Map the visible cycle field only to cycle.phase. cycle.nextChangeAt is internal program state: never create a section item for it or display it. Map visible statistics only to statistics.todayCount, statistics.totalCount, statistics.totalVolumeMl, and statistics.lastRecord. statistics.nextDailyResetAt is also internal program state. Map fan-plan display to fanPlan target/progress/reward/completed paths. Use kind=platform with profile.followerCount/profile.postCount for platform totals.",
